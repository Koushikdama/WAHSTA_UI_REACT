...
type ArchivedTab = 'following' | 'followers' | 'groups';

const ArchivedChats = () => {
  const navigate = useNavigate();
  const { chats, messages, toggleArchiveChat, users } = useApp();
  const [activeTab, setActiveTab] = useState<ArchivedTab>('following');
  ...
  const TabButton = ({ id, label }: { id: ArchivedTab, label: string }) => (
      <button 
        onClick={() => setActiveTab(id)}
        className={`flex-1 py-3 text-sm font-medium uppercase transition-all relative ${
            activeTab === id 
