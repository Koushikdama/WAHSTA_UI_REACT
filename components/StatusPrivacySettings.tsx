...
import { StatusPrivacyType, User } from '../types';

const StatusPrivacySettings = () => {
  const navigate = useNavigate();
  const { statusPrivacy, setStatusPrivacy, chats, users } = useApp();
  const [selectedPrivacy, setSelectedPrivacy] = useState<StatusPrivacyType>(statusPrivacy);
  ...
  const PrivacyOption = ({ type, label, sub, count }: { type: StatusPrivacyType, label: string, sub?: string, count?: number }) => (
      <div 
